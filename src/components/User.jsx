...
function Comment(props) {
    ...
    {props.items && props.items.map((item)=>
      ...
      <td>{item.email}</td>
      <td>{item.address && item.address["street"]}, {item.address && item.address["suite"]}, {item.address && item.address["city"]} , {item.address && item.address["zipcode"]} </td>
      <td>{item.address && item.phone}</td><td>{item.address && item.website}</td><td><DeleteIcon className='del-btn' onClick={()=>props.del(item.id)}/></td>
      </tr>)
     )}
     </tbody>
     </table>
